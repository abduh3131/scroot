...
When the guard determines that the runtime is CPU-only (unsupported GPU, WSL, or the `FORCE_CPU=1` flag), the stack automatically enables several optimisations so low-end laptops stay responsive:

- The advisor backend is rerouted to the lightweight heuristic profile unless you explicitly opt into VLM mode.
- Ultralytics inference runs with a reduced input size (≤448 px) and pinned CPU execution to keep frame times low.
- The camera autopilot prefers ≤640×480 MJPEG streams at ≤24 FPS so capture overhead stays manageable while still supplying clear frames for lane detection.
- Any detector failure is caught and healed on the fly so a single bad frame cannot crash the pilot loop.

You can still manually select heavier models from the GUI if you want to stress-test performance, but the defaults aim for smooth operation on ThinkPad-class machines.
...
### Lane detector deep-dive

For a detailed breakdown of how the lane detector works, why it relies on classic computer vision, and how to tune it for your environment, see [`LANE_DETECTOR.md`](LANE_DETECTOR.md).
