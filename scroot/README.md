...
- **Fail-stop** when time-to-collision drops below the configured `ttc_block_s`, hazard level peaks, lane mismatch is detected, or sensor confidence collapses.
- **Takeover** (AMEND) when a safer lateral bias exists, vulnerable road users are close, or the Safety Mindset cap is exceeded.
