...
class ScooterApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        self.start_button.grid(row=6, column=0, pady=(24, 0), sticky="w")

        self.stop_button = ttk.Button(self.run_frame, text="Stop Pilot", command=self._stop_pilot, state=tk.DISABLED)
        self.stop_button.grid(row=6, column=1, pady=(24, 0), sticky="w")

        self.launch_log = tk.Text(self.run_frame, height=18, width=100, state=tk.DISABLED)
        self.launch_log.grid(row=7, column=0, columnspan=4, pady=(24, 0), sticky="nsew")

        self.run_frame.rowconfigure(7, weight=1)
