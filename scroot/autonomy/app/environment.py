...
from autonomy.app.profiles import AdvisorModelProfile, DependencyProfile
...
def prepare_advisor_environment(
    profile: AdvisorModelProfile,
    hardware: HardwareProfile,
    logger: Callable[[str], None],
) -> None:
    """Preload advisor assets or log guidance based on the selected backend."""

    logger(
        "Preparing advisor profile '%s' for environment %s (%s)"
        % (profile.key, hardware.os_name, hardware.environment)
    )

    if profile.backend == "lite":
        logger("Lightweight advisor selected; no additional model downloads required.")
        return
    ...
    try:  # pragma: no cover - heavy optional dependency
        ...
        logger(f"Skipping advisor preparation; transformers not available: {exc}")
        return

    try:
        logger(f"Caching vision encoder {profile.advisor_image_model}...")
        BlipProcessor.from_pretrained(profile.advisor_image_model)
        BlipForConditionalGeneration.from_pretrained(profile.advisor_image_model)
    except Exception as exc:  # pragma: no cover - depends on HF availability
        logger(f"Unable to cache vision model: {exc}")

    try:
        logger(f"Caching language model {profile.advisor_language_model}...")
        AutoTokenizer.from_pretrained(profile.advisor_language_model)
        AutoModelForSeq2SeqLM.from_pretrained(profile.advisor_language_model)
    except Exception as exc:  # pragma: no cover
        logger(f"Unable to cache language model: {exc}")
