...
ADVISOR_MODEL_PROFILES: Dict[str, AdvisorModelProfile] = {
    "ultra_light": AdvisorModelProfile(
        key="ultra_light",
        label="Ultra-Light",
        description="Heuristic advisor that runs without torch/transformers for CPU-only hosts.",
        advisor_image_model="",
        advisor_language_model="",
        backend="lite",
    ),
...
def recommend_profiles(
...
) -> tuple[ModelProfile, DependencyProfile, AdvisorModelProfile]:
    ...
    advisor_key = "normal"
    if not profile.has_cuda or profile.compute_tier == "lightweight":
        advisor_key = "ultra_light"
    elif profile.compute_tier == "standard":
        advisor_key = "light"
    elif profile.compute_tier == "performance":
        advisor_key = "heavy"

    advisor_profile = ADVISOR_MODEL_PROFILES.get(advisor_key, ADVISOR_MODEL_PROFILES["ultra_light"])

    return model_profile, dep_profile, advisor_profile
