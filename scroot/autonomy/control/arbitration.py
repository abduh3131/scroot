...
class RuleBasedAdvisor:
    ...
    def __init__(self, config: AdvisorRuntimeConfig, max_vehicle_speed_mps: float) -> None:
        self.config = config
...
class SafetyGate:
    ...
    def apply(
        ...
        if nearest_distance is not None and nearest_distance < caps.min_clearance_m:
            throttle = 0.0
            brake = max(brake, 0.6)
...
class ControlArbiter:
    ...
    def __init__(
        ...
        self._advisor = RuleBasedAdvisor(advisor_config, max_vehicle_speed)
        self._gate = SafetyGate(max_vehicle_speed, context_config, intent_config)
