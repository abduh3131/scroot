...
class AutonomyPilot:
    ...
    def __init__(
        ...
        self._low_resource = is_low_resource_runtime() or self._runtime_device == "cpu"
        ...
        if self._low_resource:
            preferred_mode = AutoCameraMode(
                fourcc="MJPG",
                width=min(preferred_mode.width, 640),
                height=min(preferred_mode.height, 480),
                fps=min(preferred_mode.fps, 24),
            )
        try:
        ...
        if self.config.advisor_enabled:
            ...
            if self._runtime_device == "cpu":
                advisor_device = "cpu"
            if self._low_resource and self.config.advisor_backend.lower() != "lite":
                logging.info(
                    "Low-resource runtime detected; forcing advisor backend '%s' to lightweight mode.",
                    self.config.advisor_backend,
                )
                self.config.advisor_backend = "lite"
                self.config.advisor_image_model = ""
                self.config.advisor_language_model = ""
    ...
    def run(self) -> Iterator[ActuatorCommand]:
        ...
        for success, frame in frame_iterator:
            ...
            try:
                ...
                logging.exception("Object detector error; continuing with empty summary: %s", exc)
                perception_summary = PerceptionSummary(
                    objects=[], frame_size=(frame.shape[1], frame.shape[0])
                )
            try:
                lane_observation = self._lane_detector.detect(frame)
            except Exception as exc:
                logging.exception("Lane detector error; reverting to empty observation: %s", exc)
