...
def apply_runtime_guard(preferred_device: str = "auto") -> str:
    ...
    with _STATE_LOCK:
        ...
        if device == "cpu":
            ...
            os.environ.setdefault("AUTONOMY_LOW_RESOURCE_HINT", "1")
            try:
                import torch  # type: ignore
...
def is_low_resource_runtime() -> bool:
    """Return True when the guard determined a CPU-only execution path."""

    hint = os.environ.get("AUTONOMY_LOW_RESOURCE_HINT")
    if hint and hint != "0":
        return True
    with _STATE_LOCK:
        if _STATE.device != "cpu":
            return False
        return _STATE.reason in {
            "wsl_forced_cpu",
            "insufficient_sm",
            "cuda_unavailable",
            "torch_missing",
