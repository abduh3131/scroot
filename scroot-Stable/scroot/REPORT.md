...
5. **Lane-Aware Navigator** – Lane curvature/offset measurements are now injected back into the navigator’s steering bias so the pilot hugs available lanes or shoulders automatically, while lane-induced hazard penalties are capped to keep the scooter from braking unnecessarily when a clip momentarily misaligns.
...
4. **Auto-calibrated horizon + warp** runs a lightweight Hough transform on the binary mask to find the dominant rails, rebuilds the trapezoid for the perspective transform, and blends it over time so recordings from different cameras remain aligned automatically.
