...
## Overview
The scooter autonomy stack now runs on Python 3.8 and newer, spans Jetson Orin to desktop GPUs, and offers a streamlined desktop app. Sensor ingestion flows straight from ROS: `bridge/ai_input_bridge.py` subscribes to `/sensor_hub/data`, converts each SensorHub packet into `SensorSample` + `LidarSnapshot` objects, feeds them into `AutonomyPilot`, and mirrors `~/scroot/runtime_inputs/` for the GUI/offline tools when requested. The GUI scrolls smoothly on every tab, applies Jetson-specific CUDA defaults when that hardware is detected, and stores state between sessions. The **Media Test** tab lets you upload any road clip or still photo, run it through the entire pilot, and save a video with detection boxes and narration to any folder you pick, with a Stop button so you can switch clips instantly. The YOLO stack can be forced to run on CPU when CUDA kernels are missing, the advisor narration is a simple checkbox away, and hazards remain capped so the pilot eases back on speed instead of panic-braking when a video clip briefly misaligns.

## Program Features to Date
1. **Autonomy Pilot Core** – Camera ingestion, YOLO-based object detection, navigator, controller, safety mindset, telemetry logging, and riding companion narration all run inside `AutonomyPilot`.
...
3. **Jetson-Oriented Optimizations** – The GUI auto-detects Jetson Orin hardware, switches to CUDA-ready dependency stacks, and lowers resolution/FPS so inference stays smooth.
...
5. **YOLO-First Visualization** – The live overlay focuses on detection boxes and telemetry; actuator commands stay tied to fused perception and operator goals.
...
8. **Documentation Refresh** – README, GUI guide, and this report now describe the new toggles, simplified launch flow, and offline replay in approachable language.
