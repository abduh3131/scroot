...
- CPU/GPU acceleration toggle plus an advisor switch so you can run entirely on CPU hardware and silence narration when you need the lightest loop.
- Live launch dashboard with camera feed, YOLO overlays, and arbiter status plus LiDAR telemetry.
...
### Fast path (Jetson Orin)

```bash
cd ~/scroot/scroot-Stable/catkin_ws
./start_sensor_interface.sh             # rebuild + launch /sensor_hub/data
tmux new -s scroot_gui -d "cd ~/scroot/scroot-Stable && ./run_full_stack.sh"
```

The tmux session opens dedicated windows for the sensors, visualizer, dual YOLO node, AI bridge, and GUI. The GUI auto-connects to `~/scroot/runtime_inputs/camera.jpg` as soon as the SensorHub bridge starts writing frames.
...
3. **Open the *Launch* tab** to start the autonomy stack. The GUI auto-fills the SensorHub snapshot path (`~/scroot/runtime_inputs/camera.jpg`) and will auto-start the pilot as soon as the bridge writes new frames. You can still press **Start Pilot** manually if you prefer. The live feed shows the current frame, YOLO bounding boxes, safety-arbiter verdicts, and a LiDAR telemetry row that summarizes the closest/median fused ranges plus ultrasonic distance pulled from `~/scroot/runtime_inputs/lidar.npy` + `sensor_meta.json`. Use the messaging panel to monitor ALLOW/AMEND/BLOCK decisions and send natural-language directives back. The launch controls expose a lightweight **Acceleration Mode** combo (Auto, CUDA, or CPU), Safety Mindset toggle, Ambient cruising toggle, persona picker, and an **Enable Advisor** checkbox so you can silence narration when you need the lightest possible loop.

4. **Use the *Media Test* tab** when you want to sanity-check the AI without riding the scooter. Point it at any road clip (`.mp4`, `.mov`, `.avi`, `.mkv`) or still photo (`.png`, `.jpg`, `.bmp`). The tab reuses your current launch settings, replays the media through the full pilot stack, and writes a new video with YOLO boxes and captions burned into every frame. Pick a folder or filename to save the overlay anywhere you want (or leave it blank to stick with `logs/media_tests/`). The preview panel streams the most recent overlay so you can watch the run while it renders, and a dedicated **Stop** button lets you cancel the export, discard the partial file, and switch clips instantly.
