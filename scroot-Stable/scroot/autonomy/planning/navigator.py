...
class Navigator:
    ...
    def plan(
        ...
        if lane_estimate:
            ...
            lane_bias = -lane_estimate.recommended_bias
            lane_weight = min(1.0, 0.2 + lane_estimate.confidence * 0.75)
            ...
            if lane_estimate.confidence > 0.4 and self._vehicle:
                offset_ratio = abs(lane_estimate.lateral_offset_m) / max(
                    self._vehicle.required_lateral_clearance(),
                    0.1,
                )
                lane_offset_hazard = 0.0
                if offset_ratio > 1.05:
                    lane_offset_hazard = min(0.18, (offset_ratio - 1.0) * 0.12)
                if lane_offset_hazard > hazard_level:
                    metadata["lane_offset_hazard"] = lane_offset_hazard
                hazard_level = max(hazard_level, lane_offset_hazard)
        ...
        if lane_weight > 0.0:
            steering_bias = float(
                np.clip((1.0 - lane_weight) * steering_bias + lane_weight * lane_bias, -1.0, 1.0)
            )
            metadata["lane_bias_applied"] = lane_bias
            metadata["lane_weight"] = lane_weight

        desired_speed = self.config.target_speed_mps * (1.0 - 0.6 * hazard_level)
        if hazard_level < 0.5:
            desired_speed = max(desired_speed, self.config.target_speed_mps * 0.55)
        goal_context = "cruise"
